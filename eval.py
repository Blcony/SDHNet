...
def evaluate_oasis(args, model, steps):
    for datas in args.dataset_test:
        eval_path = join(args.eval_path, datas)
...
def evaluate_mindboggle(args, model, steps):
    for datas in args.dataset_test:
        eval_path = join(args.eval_path, datas)
        if not os.path.isdir(eval_path):
            os.makedirs(eval_path)
        ...
        Dice, Jacc, Jacb = [], [], []
        eval_dataset = datasets.MindboggleTest(args)
        if args.local_rank == 0:
            print('Dataset in evaluation: %s' % datas, file=f, flush=True)
            print('Image pairs in evaluation: %d' % len(eval_dataset), file=f, flush=True)
            print('Evaluation steps: %s' % steps, file=f, flush=True)
...
def evaluate(args, model, steps=100000):
    if args.dataset == 'liver':
        ...
        evaluate_mindboggle(args, model, steps)
    elif args.dataset == 'oasis':
        evaluate_oasis(args, model, steps)
